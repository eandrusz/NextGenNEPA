...
d <- readRDS(here("In_Progress","rosetta_calibration","data","MiFish.salmonidonly.mockdata.RDS")) %>% 
  mutate(time = 1,
         creek = 1,
         biol = 1) 
...
Mock <- d %>% filter(site %in% c("MCC")) %>% filter(b_proportion > 0 & Nreads > 0) 

Observation <- a %>% 
  filter(species %in% Mock$species) %>% 
  filter(species != "Salmo trutta") %>%  #for now; very rare
...
#fit model
    ...
    pivot_wider(names_from = speciesname, values_from = Nreads, values_fill = 0) %>% 
      pivot_longer(cols = str_which(names(.), "Oncorhynchus"), names_to = "speciesname", values_to = "Nreads")
...
```{r Bayesian Model Estimation in Stan}
      ...
      #full Bayesian model
      stanMod = stan(file = here("In_Progress","rosetta_calibration","quant_metabar_rosetta_noEta.stan") ,data = stan_data,
