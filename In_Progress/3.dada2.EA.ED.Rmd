...
params:
  noprimerfolder: 
    value: C:/Users/erdag/NextGenNEPA_local/Output/cutadapt_output/run5_20220221/noprimers/MiFish
  hash: TRUE
  trimming.length.Read1: 150
  trimming.length.Read2: 150
  merge.overlap: 100
  metadata: C:/Users/erdag/NextGenNEPA_local/Output/cutadapt_output/run5_20220221/noprimers/MiFish/cutadapt_output_metadata_MiFish.csv
  output.folder: C:/Users/erdag/github/NextGenNEPA-EA/Output/dada2_output/run5_20220221/MiFish
...
#sample.metadata <- sample.metadata[-10,]
...
sample.metadata %>%
  ...
  mutate (outFs = pmap_dfr(.l= list (file1, filtF1, file2, filtR1),
                           .f = function(a, b, c, d) {
                             filterAndTrim(a,b,c,d,                                           truncLen=c(params$trimming.length.Read1,params$trimming.length.Read2),
...
after.filter %>% 
  ...
  mutate(
    ...
    derepR1 = map(filtR1, derepFastq),
    dadaF1  = map2(derepF1,errF1, ~ dada(.x, err = .y, multithread = FALSE)),  # dada2
    dadaR1  = map2(derepR1,errR1, ~ dada(.x, err = .y, multithread = FALSE))) -> output.dada2

write_rds(output.dada2, file = "output.dada2.rds")

output.dada2 %>%
     mutate(mergers = pmap(.l = list(dadaF1,derepF1, dadaR1,derepR1),                 # merge things
                   .f = mergePairs,
                   minOverlap = params$merge.overlap)) -> merged
...
seqtab.nochim <- removeBimeraDenovo(seqtabF, method="consensus", multithread=FALSE)
dim(seqtab.nochim)
