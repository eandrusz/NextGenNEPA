...
We have completed cutadapt, split by marker, and ran dada2. Now we will denoise all the COI data. 

```{r load merged COI asv table, hash key, metadata}
metadata <- read.csv(here("Input", "sequencing_metadata_files", "master_sequencing_datasheet_20211026.csv"))

# depending on whether using a single run or all the runs combined, load the ASV.table and Hash.key
# and select relevant metadata
if (is.numeric(run.num)) {
  ASV.table <- read.csv(here("Output", "dada2_output", paste0("run",run.num), marker, "ASV_table.csv"))
  Hash.key <- read.csv(here("Output", "dada2_output", paste0("run",run.num), marker, "hash_key.csv"))
  metadata <- metadata %>% 
    filter(Sequencing.run == run.num) %>% 
    filter(Locus == marker)
} else {
  ASV.table <- read.csv(here("Output", "dada2_output", "20211116.combined.COI.ASV.table.csv"))
  Hash.key <- read.csv(here("Output", "dada2_output", "20211116.combined.COI.hash.key.csv"))
  metadata <- metadata %>% filter(Locus == marker)
}
...
# run 1 doesn't have technical replicates - they are pooled... so use "old_tech_rep"
metadata[metadata$Type == "old_tech_rep",]$Type <- "sample"

# remove maya's samples
metadata <- metadata %>% 
  filter(Type != "mgy")

# keep only ASVs and hashes in the samples for this run and our samples
ASV.table <- ASV.table %>% 
