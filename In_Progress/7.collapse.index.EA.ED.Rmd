...
COI.ASV.table <- read_csv(paste0(here("Output", "dada2_output", "20211120.combined.COI.ASV.table.csv")))
MiFish.ASV.table <- read_csv(paste0(here("Output", "dada2_output", "20211120.combined.MiFish.ASV.table.csv")))
MiMammal.ASV.table <- read_csv(paste0(here("Output", "dada2_output", "20211120.combined.MiMammal.ASV.table.csv")))
...
COI.annotations <- readRDS(file=paste0(here("Output","classification_output"),"/20211119.COI.all.gs.previous.hashes.annotated.rds"))
MiFish.annotations <- readRDS(file=paste0(here("Output","classification_output"),"/20211120.MiFish.all.gs.previous.hashes.annotated.rds"))
MiMammal.annotations <- readRDS(file=paste0(here("Output","classification_output"),"/20211120.MiMammal.all.gs.previous.hashes.annotated.rds"))
...
```{r COI}
# this taxonomy file is the "good" annotations to species and genus level so let's get rid of the other junk for right now 
COI.simple.annotations <- COI.annotations %>% select(representative, taxon, genus, species) %>% rename(Hash = representative)
COI.ASV.taxon <- COI.ASV.table %>% 
  left_join(COI.simple.annotations, by = "Hash") %>% 
  select(-Locus)

# ok so that has everything.. let's remove (and save) ASVs that don't have an annotation 
COI.ASV.no.taxon <- COI.ASV.taxon[is.na(COI.ASV.taxon$taxon),]
# and same - keep things that do have an annotation (reminder to species or genus level so "good" annotation)
COI.ASV.yes.taxon <- COI.ASV.taxon[! is.na(COI.ASV.taxon$taxon),]

# compare number of ASVs and reads that are assigned to species and genus (or not)
COI.n.ASV.no.taxon <- length(unique(COI.ASV.no.taxon$Hash))
COI.n.reads.ASV.no.taxon <- sum(COI.ASV.no.taxon$nReads)
COI.n.ASV.yes.taxon <- length(unique(COI.ASV.yes.taxon$Hash))
COI.n.reads.ASV.yes.taxon <- sum(COI.ASV.yes.taxon$nReads)
COI.percent.ASV.no.taxon <- COI.n.ASV.no.taxon/(COI.n.ASV.no.taxon+COI.n.ASV.yes.taxon)*100 
COI.percent.reads.ASV.no.taxon <- COI.n.reads.ASV.no.taxon/(COI.n.reads.ASV.no.taxon+COI.n.reads.ASV.yes.taxon)*100 
# ok so 99% of ASVs are not to genus or species and only 96% of *reads* are not assigned to genus or species
# that's not great, we can go back later and use the whole classification and see if we can get more info (family, order, etc.)

# let's take only the ASVs that are annotated to something and collapse taxa (add reads together of ASVs that go to the same taxon)
COI.by.taxa <- COI.ASV.yes.taxon %>% 
  select(-Hash) %>% # we don't need the hash identifier anymore
  group_by(Sample_name, taxon) %>% # for each sample that has multiple asvs that assign to the same taxa...
  summarise(tot = sum(nReads))

COI.by.taxa.pivot <- COI.by.taxa %>% 
  pivot_wider(names_from = Sample_name, values_from =  tot) %>% # switch to rows (taxa) by columns (samples)
  column_to_rownames(var="taxon")

# we can also do this at the genus level 
COI.by.genus <- COI.ASV.yes.taxon %>% 
  select(-Hash) %>% # we don't need the hash identifier anymore
  group_by(Sample_name, genus) %>% # for each sample that has multiple asvs that assign to the same taxa...
  summarise(tot = sum(nReads))

COI.by.genus.pivot <- COI.by.genus %>% 
  pivot_wider(names_from = Sample_name, values_from =  tot) %>% # switch to rows (taxa) by columns (samples)
  column_to_rownames(var="genus")

# ok but we really should be using eDNA index instead of reads to be comparing things (LATER LET'S GO BACK AND DO BETTER)
source(here("functions", "eDNAindex.R"))
COI.ASV.index <-  eDNAindex(COI.ASV.table, Sample_name, Hash, nReads) # here use everything whether it was assigned taxonomy or not
COI.taxa.index <- eDNAindex(COI.by.taxa, Sample_name, taxon, tot)
COI.genus.index <- eDNAindex(COI.by.genus, Sample_name, genus, tot)

COI.genus.index.pivot <- COI.genus.index %>% 
  pivot_wider(names_from = Sample_name, values_from =  Normalized.reads) %>% # switch to rows (taxa) by columns (samples)
  column_to_rownames(var="genus")

# let's be really bad and just pool biological replicates right now....
COI.by.genus.bio <- COI.by.genus %>% 
  separate(Sample_name, into=c("marker","date","creek","site","bio_rep"), remove = FALSE) %>% 
  unite("original_sample", "marker","date","creek","site")

COI.by.genus.pooled.bio <- COI.by.genus.bio %>% 
  separate(Sample_name, into=c("marker","date","creek","site","bio_rep"), remove = FALSE) %>% 
  unite("original_sample", "marker","date","creek","site") %>% 
  group_by(original_sample, genus) %>% # for each sample that has multiple asvs that assign to the same taxa...
  summarise(tot = sum(tot)) 

COI.genus.bio.index <- eDNAindex(COI.by.genus.bio, original_sample, genus, tot)
COI.genus.bio.index2 <- eDNAindex(COI.by.genus.bio, original_sample, genus, tot, bio_rep)
COI.genus.pooled.bio.index <- eDNAindex(COI.by.genus.pooled.bio, original_sample, genus, tot)

COI.genus.pooled.bio.index.pivot <- COI.genus.pooled.bio.index%>% 
  pivot_wider(names_from = original_sample, values_from =  Normalized.reads) %>% # switch to rows (taxa) by columns (samples)
  column_to_rownames(var="genus")
