...
*IMPORTANT* won't work unless manually put in full file path to params.txt file at bottom - do that then you can run this by just clicking knit.
...
Sys.setenv(PATH = paste(old_path, "C:/Users/erdag/AppData/Local/Programs/Python/Python39/Scripts", sep = ":"))

setwd("C:/Users/erdag/github/NextGenNEPA-EA/In_Progress")
...
fastqfolder <- paste0("C:/Users/erdag/NextGenNEPA_local/Input/fastq_files","/run",params$run.num)

sequencingmetadatafile <- paste0(here("Input","sequencing_metadata_files/"),"/metadata-input-run",params$run.num, ".csv") 

outputfolder <- paste0("C:/Users/erdag/NextGenNEPA_local/Output/","cutadapt_output","/run",params$run.num,"_",format(Sys.Date(), "%Y%m%d"))
...
```{bash}
bash test.bash.sh "C:/Users/erdag/NextGenNEPA_local/Output/cutadapt_output/run5_20220221/params.txt"
```
