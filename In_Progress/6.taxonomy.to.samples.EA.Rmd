...
write.csv(MiMammal.by.species, here("Output","combined_species_tables","20220210_MiMammal.csv"), row.names = FALSE)
```
...
write.csv(MiFish.by.species, here("Output","combined_species_tables","20220210_MiFish.csv"), row.names = FALSE)
...
COI.by.species <- COI.ASV.yes.taxon %>% 
  select(-Hash) %>% # we don't need the hash identifier anymore
  #filter(species != "") %>% 
  group_by(Sample_name, class, order, family, genus, species) %>% # for each sample that has multiple asvs that assign to the same taxa...
  summarise(tot = sum(nReads))

write.csv(COI.by.species, here("Output","combined_species_tables","20220210_COI.csv"), row.names = FALSE)
