...
a <- readRDS("data/COI.incomplete.all.envirodata.RDS")

d <- readRDS(here("In_Progress","rosetta_calibration","data","COImockdata.RDS")) %>% 
  mutate(time = 1,
         creek = 1,
         biol = 1) %>% 
  filter(tech == 1)
...
Mock <- d %>% filter(site %in% c("MCB")) %>% filter(b_proportion > 0 & Nreads > 0) 
...
#fit model
      ...
      #likelihood
      M <- stan_model(file=here("In_Progress","rosetta_calibration","quant_metabar_rosetta_noEta.stan"))
