...
taxonomy <- taxonomy %>% 
  ...
  mutate(., species = case_when(species == "Lampetra tridentata" ~ "Entosphenus tridentatus",
                                TRUE ~ species))  
...
seqs.tax[80:82,2] <- start.metadata[121:123,2]
...
ggplot(mock.data, aes(x = tech, y = b_proportion, fill = species)) +
  ...
  theme_bw() +
  labs(x="Technical Replicate", y="Proportion of Reads", title = "Expected Proportions") +
