---
title: "COI_visualizing_for_elif"
author: "Eily Allan"
date: "11/17/2021"
...
metadata <- metadata %>% filter(Locus == "COI")

ASV.table <- read.csv("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA_EA/Output/dada2_output/20211129.combined.COI.ASV.table.csv")

# check that all samples are in ASV.table as in metadata file
dim(metadata)[1] == length(unique(ASV.table$Sample_name))

Hash.key <- read.csv("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA_EA/Output/dada2_output/20211129.combined.COI.hash.key.csv")
...
# for now, just remove kangaroo, technical replicates, and maya's samples
metadata <- metadata %>% 
  filter(Type == "sample")

# special case - remove the barnes up 2 old that had a whole bug on it
...
#run 2 has two different folders, each with classifications... 
#ignoring the forwards and backwards ones right now... damn we need to figure that out (when in pipeline to RC)
run2.classif <- readRDS("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA/Output/hashes_to_taxonomy_output/run2/COI/hashes.annotated.rds")
run2.2.classif <- readRDS("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA/Output/hashes_to_taxonomy_output/run2_rerun/COI/hashes.annotated.rds")

# run 3 
run3.classif <- readRDS("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA/Output/hashes_to_taxonomy_output/run3/COI/hashes.annotated.rds")

# run 4 which has the insect classifier and our blast tree
run4.blast.classif <- readRDS("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA/Output/hashes_to_taxonomy_output/run4_rerun/COI_blast_classifier/hashes.annotated.rds")
run4.insect.classif <- readRDS("/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA/Output/hashes_to_taxonomy_output/run4_rerun/COI_old_classifier/hashes.annotated.rds")

# let's just blindly merge them all for right now... this could be bad LOL
# first have to remove teh accession column from run4.blast.classif
run4.blast.classif <- run4.blast.classif %>% select(-accession)
...
big.key <- rbind(run2.classif, run2.2.classif, run3.classif, run4.blast.classif, run4.insect.classif)
...
raw.pca.to.plot <- cbind(metadata, pca.raw.bc.MDS1, pca.raw.bc.MDS2)
...
ggplot(raw.pca.to.plot, aes(x=pca.raw.bc.MDS1, y=pca.raw.bc.MDS2)) +
  ...
  ggtitle('COI - Raw reads combined to taxa - Bray Curtis')
...
all.distances.to.plot$Distance.type <- all.distances.to.plot$Distance.type  %>% fct_relevel("Biol.replicates", "Same.datecreek.Diff.site", "Same.date.Diff.creek", "Same.creek.different.date", "Different.creek.date")
...
ggsave(file="/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA_EA/Output/COI.allruns.taxalevel.visual.anova.png", dpi = "retina")
